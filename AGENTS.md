...
- 客户端页面也展示二维码和链接复制按钮
- 客户端网页左上角和浏览器标题显示分享来源电脑的系统主机名，读取失败时回退显示当前访问地址的主机部分
...
- Windows 再次启动应用时必须唤起已有实例的窗口，包括隐藏和最小化状态；不能创建第二个托盘或服务，也不能重启正在进行的分享
...
- 上传文本和上传/共享文件使用弹窗
- 使用说明通过居中弹窗展示，尺寸受当前视口限制；内容超高时在弹窗内滚动，关闭按钮保持可见
...
- `src-tauri/src/main.rs`
  - Tauri 入口
  - 暴露桌面命令，如启动服务、停止服务、服务状态、系统文件选择器、登记本机文件、发布管理端文本、移除条目、管理端下载、检查更新
  - 实现系统托盘、窗口关闭隐藏、退出和 updater 入口

- `src-tauri/src/server.rs`
  - Rust HTTP 服务
  - 提供客户端页面、普通客户端 HTTP 接口、SSE、下载状态 SSE、Range 文件下载、客户端上传保存、网卡地址枚举
...
- `/api/events`
  - SSE 实时推送列表变化
  - 主动停止分享时，先发送 `share-stopped` 事件再结束 SSE；Web 端显示“分享已停止”并停止自动重连，重新开启后手动刷新

- `/api/download-events`
  - SSE 实时推送 Web 端下载状态，管理端用于展示正在下载和速率
  - 停止分享时同样发送 `share-stopped` 并结束连接
...
- HTTP 服务的 CORS 全部放行，不使用来源白名单限制跨域；管理端能力通过 Tauri command 隔离。
- 管理端写操作必须通过 Tauri command，不暴露局域网 HTTP 管理接口
...
- 上传中间文件使用应用数据目录内按服务实例隔离的临时目录；正常退出时只清理自己拥有的目录，禁止按文件名扫描并删除 Downloads 中的文件
- 发布新版本时，changelog/release notes 必须基于“上一个发布版本 tag 到当前发布 tag”的完整变更来整理，列出面向用户的重要功能更新和体验改进；不要只写本次最后一个提交，也不需要罗列实现细节或内部修改细节。
- 对 GitHub Release 的发布状态和构建情况只做简洁说明，不要在回复中展开过多细节。
